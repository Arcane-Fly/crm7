...
export function useLMS() {
  ...
  return {
    ...
    updateCourse: (id: string, data: Partial<CourseCreate>) => updateCourse({ match: { id }, data }),
    isCreatingCourse,
    isUpdatingCourse,

    // Enrollment Mutations
    createEnrollment: (data: EnrollmentCreate) => createEnrollment({ data }),
    updateEnrollment: (id: string, data: Partial<EnrollmentCreate>) => updateEnrollment({ match: { id }, data }),
