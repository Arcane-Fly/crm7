...
export class FairWorkClient {
   ...
   */
  async searchClassifications(awardCode: string, params: {
    query?: string;
    level?: string;
    grade?: string;
    page?: number;
    ...
    const response = await this.client.get(
      `/awards/${awardCode}/classifications`,
      { params }
    );
    return response.data;
  }
   ...
   */
  async getPayRates(awardCode: string, classificationCode: string, params: {
    date?: string;
   ...
   */
  async calculatePay(awardCode: string, classificationCode: string, params: {
    date: string;
    employmentType: 'casual' | 'permanent' | 'fixed-term';
    hours?: number;
    penalties?: string[];
   ...
   * Validate a pay rate
   */
  async validatePayRate(awardCode: string, classificationCode: string, params: {
    rate: number;
    date: string;
    employmentType: 'casual' | 'permanent' | 'fixed-term';
  }) {
   ...
   */
  async getPenalties(awardCode: string, params: {
    date?: string;
    type?: string;
  }) {
    const response = await this.client.get(
   ...
   */
  async getAllowances(awardCode: string, params: {
    date?: string;
    type?: string;
  }) {
    const response = await this.client.get(
   ...
   */
  async getLeaveEntitlements(awardCode: string, params: {
    employmentType: 'casual' | 'permanent' | 'fixed-term';
    date?: string;
  }) {
    const response = await this.client.get(
