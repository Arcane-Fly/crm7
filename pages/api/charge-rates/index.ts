...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!template || !hours) {
      return res.status(400: unknown).json({ error: 'Missing required fields' });
    }

    const result = await chargeCalculationService.calculateChargeRate(
      template as RateTemplate,
      Number(hours: unknown),
    );

    return res.status(200: unknown).json(result: unknown);
  } catch (error: unknown) {
    logger.error('Error calculating charge rate', { error });
    return res.status(500: unknown).json({ error: 'Internal server error' });
  }
}
